...
export function billReadyEmail({ tenantName, propertyName, roomNumber, month, rentAmount, electricityAmount, electricityDetails = [], otherCharges, totalAmount, balance, dueDate, invoiceUrl }: {
  tenantName: string; propertyName: string; roomNumber: string; month: string; rentAmount: string; electricityAmount: string; electricityDetails?: string[]; otherCharges: string; totalAmount: string; balance: string; dueDate: string; invoiceUrl: string;
}): string {
  ...
  const electricityDetailRows = electricityDetails.map((detail) => `<tr><td colspan="2" style="padding:0 0 7px;color:#64748b;font-size:13px">${escapeHtml(detail)}</td></tr>`).join("");
  return `<div style="font-family:${FONT_STACK};max-width:620px;margin:0 auto;padding:28px;background:#f8fafc">
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:28px">
      ...
      <table style="width:100%;border-collapse:collapse;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;font-size:15px">${row("Rent", rentAmount)}${row(electricityDetails.length > 1 ? "Electricity (your charge)" : "Electricity", electricityAmount)}${electricityDetailRows}${otherCharges !== "₹0" ? row("Other charges", otherCharges) : ""}${row("Total", totalAmount, true)}${row("Balance due", balance, true)}${row("Due date", dueDate)}</table>
