...
import { useAdminTenant, useApproveAdminTenant, useRejectAdminTenant } from "@/hooks/use-admin-tenants";
...
export default function TenantDetailPage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const approve = useApproveAdminTenant();
  const reject = useRejectAdminTenant();
  ...
  function handleApprove() {
    approve.mutate(tenantId, {
      onSuccess: () => toast.success("Tenant approved"),
      onError: (err) => toast.error(err instanceof Error ? err.message : "Failed"),
    });
  }

  function handleReject() {
    reject.mutate(tenantId, {
      onSuccess: () => toast.success("Tenant rejected"),
      onError: (err) => toast.error(err instanceof Error ? err.message : "Failed"),
    });
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {tenant.status === "pending" && (
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={handleReject} disabled={reject.isPending} className="text-destructive hover:text-destructive">
      ...
      <div className="rounded-xl border bg-card p-5 shadow-xs">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            ...
            <p className="text-sm">{tenant.bedNumber ? `${tenant.roomNumber || ""}-${tenant.bedNumber}` : "Unassigned"}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-xs font-mono text-muted-foreground">{tenant.id}</p>
          </div>
        </div>
      </div>
    </div>
